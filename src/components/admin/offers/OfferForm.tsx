...
export default function OfferForm({ offerId }: { offerId?: string }) {
  ...
  useEffect(() => {
    ...
    getOfferById(offerId)
      .then((offer) => {
        if (!offer) {
          setError("This offer could not be found.");
          return;
        }
        reset({
          title: offer.title,
      ...
      .catch(() => setError("Unable to load this offer."))
  ...
  if (loading)
    return (
      <div className="flex justify-center py-20 text-leaf-muted">
  ...
  return (
    <Card className="border-leaf-border bg-white">
      <CardHeader>
        <CardTitle className="text-xl text-leaf-navy">
          {offerId ? "Edit Offer" : "Create Offer"}
        </CardTitle>
        <CardDescription>
          Set the offer details shown to visitors.
        </CardDescription>
      </CardHeader>
      <CardContent>
        {error && (
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <div className="space-y-2">
            ...
            <Input {...register("title")} className="border-leaf-border" />
            {errors.title && (
          ...
          <div className="space-y-3">
            ...
            {preview ? (
              <div className="relative aspect-video max-w-md overflow-hidden rounded-xl border border-leaf-border">
                <Image
                  src={preview}
              ...
              <label className="flex max-w-md cursor-pointer flex-col items-center justify-center gap-2 rounded-xl border border-dashed border-leaf-border bg-leaf-bg px-6 py-10 text-sm text-leaf-muted hover:bg-leaf-soft">
                <ImagePlus className="size-7 text-leaf-green-dark" />
            ...
            {preview && (
              <label className="inline-flex cursor-pointer text-sm font-medium text-leaf-green-dark hover:text-leaf-green">
                Replace image
                <input
                  type="file"
                  accept="image/jpeg,image/png,image/webp"
          ...
          <p className="rounded-lg bg-leaf-soft px-4 py-3 text-sm text-leaf-navy">
            You save:{" "}
            <strong>
              Rs. {Math.max(0, price - discountPrice).toLocaleString()}
            </strong>
          </p>
          <div className="flex gap-3 rounded-lg border border-leaf-border p-4">
            <Checkbox
              checked={isActive}
            ...
            <div>
              ...
              <p className="text-xs text-leaf-muted">
                Show this offer to users on the website.
              </p>
            </div>
          </div>
          <div className="flex justify-end gap-3 border-t border-leaf-border pt-4">
            <Link
              href="/admin/offers"
              className="rounded-lg border border-leaf-border px-4 py-2 text-sm font-medium text-leaf-navy"
            >
              Cancel
            </Link>
            <Button
              ...
              className="bg-leaf-green-dark px-4 py-2 text-white hover:bg-leaf-green"
            >
